...
async def start_payment(interaction: discord.Interaction, invoice_id: str, roblox_username: str):
    ...
    for item in inv.get("items", []):
        ...
        if p:
            pool, pname = product_pool(p), p["name"]
            break
    if not pool:
    ...
    # pick a gamepass this buyer doesn't already own (repeat buyers get the next one)
    busy = {o["gamepass_id"] for o in STATE["pending"]
            if o.get("roblox_id") == roblox_id and o.get("gamepass_id")}
    gamepass_id, status = await pick_gamepass(pool, roblox_id, exclude=busy)
    if status == "all_owned":
        await interaction.followup.send(
            f"⚠️ The account `{exact}` already owns every gamepass available for this product. "
            f"Please contact an admin to add another one.", ephemeral=True)
        await client.alert_admin(
            f"⚠️ `{exact}` owns all gamepasses for **{pname}** (order #{invoice_id}). "
            f"Add another with `/gamepass_add`.")
        return
...
@client.tree.command(name="product_set",
                     description="Add or update a product's name/price (admin)")
...
async def product_set(interaction: discord.Interaction,
    ...
    key, existing = product_for_sellauth_id(sellauth_id)
    if existing:
        if name is not None:
            existing["name"] = name
        ...
        key = str(sellauth_id)
        PRODUCTS[key] = {
            "name": name or f"Product {sellauth_id}",
            "gamepass_ids": [],
...
@client.tree.command(name="gamepass_add", description="Add a gamepass to a product's pool (admin)")
...
async def gamepass_add(interaction: discord.Interaction, sellauth_id: int, gamepass_id: int):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("Admins only.", ephemeral=True)
        return
    key, p = product_for_sellauth_id(sellauth_id)
    if not p:
        await interaction.response.send_message(
            f"No product with store ID `{sellauth_id}`. Make it first with `/product_set`.",
            ephemeral=True)
        return
    pool = _ensure_pool(p)
...
@client.tree.command(name="gamepass_remove", description="Remove a gamepass from a product's pool (admin)")
...
async def gamepass_remove(interaction: discord.Interaction, sellauth_id: int, gamepass_id: int):
    ...
    if not p:
        await interaction.response.send_message(
            f"No product with store ID `{sellauth_id}`.", ephemeral=True)
        return
    pool = _ensure_pool(p)
...
async def product_list(interaction: discord.Interaction):
    ...
    for p in PRODUCTS.values():
        price = p.get("price_robux")
        pool = product_pool(p)
        gp = ", ".join(f"`{g}`" for g in pool) if pool else "_none — add with /gamepass_add_"
        head = f"• **{p.get('name', '(unnamed)')}** — store `{p.get('sellauth_product_id')}`"
...
@client.tree.command(name="product_remove", description="Remove a Robux product (admin)")
@app_commands.describe(sellauth_id="The store product ID to remove")
async def product_remove(interaction: discord.Interaction, sellauth_id: int):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("Admins only.", ephemeral=True)
        return
    key, existing = product_for_sellauth_id(sellauth_id)
    if not existing:
        await interaction.response.send_message(
            f"No product found with store ID `{sellauth_id}`.", ephemeral=True)
        return
