...
# Roblox call goes through curl_cffi impersonating Chrome.
...
def _roblox_session():
    global _roblox
    if _roblox is None:
        _roblox = _CurlSession(impersonate="chrome", timeout=20)
    return _roblox
...
class RobuxBot(discord.Client):
    ...
    async def deliver_local(self, order: dict):
        ...
        msg = product.get("deliver_message", "Danke für deinen Kauf!")
        if delivered_key:
            msg += f"\n\n**Dein Key:** `{delivered_key}`"
        elif stock is not None:
            msg += "\n\n⚠️ Stock ist leer — ein Admin meldet sich gleich."
    ...
    async def fulfill_sellauth(self, order: dict):
        ...
        if ok:
            await self.dm(discord_id,
                          f"✅ Zahlung bestätigt! Bestellung **#{inv_id}** wurde geliefert. "
                          f"Check deine Email / Order-Seite auf wezzy.store.")
        else:
            await self.dm(discord_id,
                          f"⚠️ Robux erkannt, aber Auto-Lieferung schlug fehl ({msg}). "
                          f"Ein Admin hilft dir gleich.")
        if LOG_CHANNEL_ID:
            ch = self.get_channel(LOG_CHANNEL_ID)
            if ch:
                ico = "✅" if ok else "⚠️"
...
@client.tree.command(name="buy", description="Produkt direkt mit Robux kaufen (ohne SellAuth)")
@app_commands.describe(product="Welches Produkt willst du?", roblox_username="Dein Roblox Username")
...
async def buy(interaction: discord.Interaction, product: str, roblox_username: str):
    ...
    if not prod:
        await interaction.followup.send("Unbekanntes Produkt.", ephemeral=True)
        return
    resolved = await resolve_username(client.session, roblox_username)
    if not resolved:
        await interaction.followup.send(f"Roblox-User `{roblox_username}` nicht gefunden.", ephemeral=True)
        return
    ...
    await interaction.followup.send(
        ...
        f"1. Kauf den Gamepass: {link}\n"
        f"2. Auto-Lieferung in max. {POLL_SECONDS}s.\n\n"
        f"⚠️ Kauf mit **dem** Account `{exact}` — sonst kein Match.", ephemeral=True)


@client.tree.command(name="pay", description="wezzy.store Bestellung mit Robux bezahlen")
@app_commands.describe(invoice_id="Deine SellAuth Invoice-ID", roblox_username="Dein Roblox Username")
async def pay(interaction: discord.Interaction, invoice_id: str, roblox_username: str):
    await interaction.response.defer(ephemeral=True)
    if not client.sellauth:
        await interaction.followup.send("SellAuth ist nicht konfiguriert.", ephemeral=True)
        return

    inv = await client.sellauth.get_invoice(invoice_id)
    if not inv:
        await interaction.followup.send("Invoice nicht gefunden. Stimmt die ID?", ephemeral=True)
        return
    if inv.get("status") == "completed":
        await interaction.followup.send("Diese Bestellung ist bereits bezahlt. ✅", ephemeral=True)
        return
    ...
    if not gamepass_id:
        await interaction.followup.send(
            "Für dieses Produkt ist kein Robux-Gamepass hinterlegt. Bitte Admin kontaktieren.",
            ephemeral=True)
        return
    ...
    await interaction.followup.send(
        f"🛒 Bestellung **#{invoice_id}** — {pname}\nRoblox: `{exact}`\n\n"
        f"1. Kauf diesen Gamepass: {link}\n"
        f"2. wezzy.store liefert automatisch in max. {POLL_SECONDS}s.\n\n"
        f"⚠️ Kauf mit `{exact}` — sonst kein Match.", ephemeral=True)


@client.tree.command(name="pending", description="Offene Bestellungen (Admin)")
