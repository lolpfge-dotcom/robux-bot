...
AMOUNT_CHECK = os.getenv("AMOUNT_CHECK", "true").lower() != "false"   # reject underpaid purchases
ROBLOX_FEE = float(os.getenv("ROBLOX_FEE", "0.30"))          # Roblox marketplace cut (seller keeps the rest)
...
# Embed accent line. Defaults to your Wezzy bot's embed grey (#323338).
PANEL_COLOR = int(os.getenv("PANEL_COLOR", "323338"), 16)
...
def _parse_ids(text: str):
    """Parse a comma/space separated string of IDs -> (unique_ints, bad_tokens)."""
    clean, bad, seen = [], [], set()
...
def _normalize(items):
    ...
    for t in items:
        ...
        out.append({
            ...
            "amount": currency.get("amount"),   # Robux the seller received for this sale
...
class RobuxBot(discord.Client):
    ...
    async def check_sales(self):
        ...
        for sale in sales:
            ...
            # underpayment guard: reject if the Robux received is well below expected
            if AMOUNT_CHECK and match.get("unit_price") and sale.get("amount") is not None:
                expected_net = float(match["unit_price"]) * (1 - ROBLOX_FEE)
                if sale["amount"] < expected_net * 0.95:
                    processed.add(sale["id"])              # don't re-evaluate / spam alerts
                    STATE["processed"] = list(processed)[-2000:]
                    save_state(STATE)
                    await self.alert_admin(
                        f"⚠️ **Underpaid** — order **#{match.get('sellauth_invoice_id')}**: received "
                        f"{sale['amount']} R$ on gamepass `{sale['target_id']}` but expected "
                        f"~{int(expected_net)} R$. Check that gamepass's price. **Not delivered.**")
                    await self.dm(int(match["discord_id"]),
                        f"⚠️ Your payment for order **#{match.get('sellauth_invoice_id')}** came in "
                        f"lower than expected, so it wasn't auto-delivered. Please contact an admin.")
                    continue
...
async def start_payment(interaction: discord.Interaction, invoice_id: str, roblox_username: str):
    ...
    pool, pname, qty, first_key, unit_price = [], None, 0, None, None
    ...
    await interaction.followup.send(
        ...
        f"{body}\n\n"
        f"⚠️ Buy with the account `{exact}`. Your product is delivered on your "
        f"**wezzy.store order page** automatically — keep your Discord **DMs open** "
        f"if you'd also like a confirmation message.",
        ephemeral=True)
...
def build_panel_embed() -> discord.Embed:
    """The customer-facing payment panel, styled to match the Wezzy bot embeds."""
    embed = discord.Embed(
        title="Pay with Robux",
        description="Complete your **Wezzy** order with Robux and it delivers "
                    "**automatically** — no waiting on us.",
        color=PANEL_COLOR,
    )
    embed.add_field(
        name="How it works",
        value=(
            "`1.` Press **Pay with Robux** below.\n"
            "`2.` Enter your Wezzy **Order ID** + **Roblox username**.\n"
            "`3.` Buy the gamepass link the bot sends you.\n"
            "`4.` Your order is delivered automatically."
        ),
        inline=False,
    )
    embed.add_field(
    ...
    embed.add_field(
        ...
        value="Keep your Discord **DMs open** so we can message you — your product also "
              "appears on your **wezzy.store** order page.",
        inline=False,
    )
    if PANEL_THUMB_URL:
    ...
    embed.set_footer(text="Wezzy · Robux checkout", icon_url=PANEL_THUMB_URL or None)
...
@client.tree.command(name="gamepass_add",
                     description="Add one or more gamepasses to a product/variant pool (admin)")
@app_commands.describe(sellauth_id="The store product ID",
                       gamepass_ids="One or more gamepass IDs, separated by commas or spaces",
                       variant_id="The variant ID (optional)")
async def gamepass_add(interaction: discord.Interaction, sellauth_id: int, gamepass_ids: str,
                       variant_id: int = None):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("Admins only.", ephemeral=True)
        return
    ...
    if not p:
        ...
        return
    new_ids, bad = _parse_ids(gamepass_ids)
    if bad:
        await interaction.response.send_message(
            f"⚠️ These weren't valid gamepass IDs: {', '.join(bad)}. Nothing changed.",
            ephemeral=True)
        return
    if not new_ids:
        await interaction.response.send_message("Give me at least one gamepass ID.", ephemeral=True)
        return
    pool = _ensure_pool(p)
    added, skipped = [], []
    for g in new_ids:
        (skipped if g in pool else added).append(g)
        if g not in pool:
            pool.append(g)
    try:
        save_products(PRODUCTS)
        ...
        await interaction.response.send_message(f"⚠️ Couldn't save: {e}", ephemeral=True)
        return
    msg = f"✅ Added **{len(added)}** gamepass(es) to **{p['name']}** — pool now has {len(pool)}."
    if skipped:
        msg += f"\n_(already in pool, skipped: {', '.join(str(s) for s in skipped)})_"
    await interaction.response.send_message(msg, ephemeral=True)


@client.tree.command(name="gamepass_remove",
                     description="Remove one or more gamepasses from a pool (admin)")
@app_commands.describe(sellauth_id="The store product ID",
                       gamepass_ids="One or more gamepass IDs, separated by commas or spaces",
                       variant_id="The variant ID (optional)")
async def gamepass_remove(interaction: discord.Interaction, sellauth_id: int, gamepass_ids: str,
                          variant_id: int = None):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("Admins only.", ephemeral=True)
        return
    ...
    if not p:
        ...
        return
    ids, bad = _parse_ids(gamepass_ids)
    if bad:
        await interaction.response.send_message(
            f"⚠️ These weren't valid gamepass IDs: {', '.join(bad)}. Nothing changed.",
            ephemeral=True)
    ...
    try:
        ...
        await interaction.response.send_message(f"⚠️ Couldn't save: {e}", ephemeral=True)
        return
    msg = f"🗑️ Removed **{len(removed)}** from **{p['name']}** — pool now has {len(pool)}."
    if missing:
        msg += f"\n_(not in pool: {', '.join(str(m) for m in missing)})_"
    await interaction.response.send_message(msg, ephemeral=True)


@client.tree.command(name="gamepass_set",
                     description="Replace a product's WHOLE gamepass pool at once (admin)")
@app_commands.describe(sellauth_id="The store product ID",
                       gamepass_ids="Gamepass IDs, separated by commas or spaces",
                       variant_id="The variant ID (optional)")
async def gamepass_set(interaction: discord.Interaction, sellauth_id: int,
                       gamepass_ids: str, variant_id: int = None):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("Admins only.", ephemeral=True)
        return
    key, p = find_entry(sellauth_id, variant_id)
    if not p:
        ...
        return
    clean, bad, seen = [], [], set()
    for tok in gamepass_ids.replace(",", " ").split():
        try:
            g = int(tok)
        except ValueError:
            bad.append(tok)
    ...
    try:
        ...
        await interaction.response.send_message(f"⚠️ Couldn't save: {e}", ephemeral=True)
        return
    shown = "  ".join(f"`{g}`" for g in clean) if clean else "*(empty)*"
    await interaction.response.send_message(
        f"✅ **{p['name']}** pool set to {len(clean)} gamepass(es): {shown}", ephemeral=True)


@client.tree.command(name="gamepass_clear",
                     description="Remove ALL gamepasses from a product (admin)")
@app_commands.describe(sellauth_id="The store product ID", variant_id="The variant ID (optional)")
async def gamepass_clear(interaction: discord.Interaction, sellauth_id: int, variant_id: int = None):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("Admins only.", ephemeral=True)
        return
    key, p = find_entry(sellauth_id, variant_id)
    ...
    try:
        ...
        return
    await interaction.response.send_message(
        f"🗑️ Cleared all gamepasses from **{p['name']}**. Add new ones with `/gamepass_add`.",
        ephemeral=True)
